...
def get_event_details(event_id):
    ...
    try:
        with get_db_connection() as conn:
            ...
            nestle_count, comp_count = parse_detection_data(roboflow_data)
            
            # Get image paths
            image_paths = json.loads(event['image_path'])
            
            # Look for labeled images in the detection data
            labeled_images = []
            try:
                if isinstance(roboflow_data, list):
                    for item in roboflow_data:
                        if 'labeled_image' in item:
                            labeled_images.append(item['labeled_image'])
            except Exception as e:
                logger.error(f"Error extracting labeled images from event {event_id}: {e}")
            
            # Extract detailed product data (if available)
            products = []
            try:
                if isinstance(roboflow_data, dict) and 'predictions' in roboflow_data:
                    products = roboflow_data['predictions']
                elif isinstance(roboflow_data, list) and len(roboflow_data) > 0:
                    for item in roboflow_data:
                        if 'roboflow_predictions' in item:
                            products.extend(item['roboflow_predictions'])
                        if 'dinox_predictions' in item:
                            # Convert dinox predictions to same format as roboflow
                            for pred in item['dinox_predictions']:
                ...
                logger.error(f"Error extracting products from event {event_id}: {e}")
            ...
            for event in events:
                try:
                    ...
                    if event_date in dates:
                        ...
                        # Extract counts from roboflow_data
                        if isinstance(roboflow_data, dict):
                            if 'roboflow_predictions' in roboflow_data:
                                dashboard_data['daily_data']['nestle_values'][date_index] += len(roboflow_data['roboflow_predictions'])
                            if 'dinox_predictions' in roboflow_data:
                                dashboard_data['daily_data']['competitor_values'][date_index] += len(roboflow_data['dinox_predictions'])
