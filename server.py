...
# DINO-X setup
DINOX_API_KEY = os.getenv("DINO_X_API_KEY", "35fe4ad58926f0dfdf983bf44a8a19be")
...
def save_uploaded_file(file, device_id: str) -> str:
    ...
    # Save directly to uploads folder without device subfolder
        ...
        for key, file in request.files.items():
            try:
                if file and file.filename:
                    # Save file and get only filename
                    filename = save_uploaded_file(file, device_id)
                    saved_images.append(filename)  # Store only filename
    ...
    try:
        with get_db_connection() as conn:
            ...
            try:
                ...
                # Parse image path - handle both string and JSON array
                try:
                    image_paths = json.loads(event['image_path'])
                except json.JSONDecodeError:
                    # If not JSON, treat as single string
                    image_paths = [event['image_path']]
                
                if isinstance(image_paths, str):
                    image_paths = [image_paths]

                # Extract counts and products
                nestle_count = 0
                comp_count = 0
                nestle_products = {}
                competitor_products = {}
                ...
                if isinstance(roboflow_data, dict):
                    ...
                    if 'dinox_predictions' in roboflow_data:
                        ...
                        comp_count = sum(competitor_products.values())
                    if 'counts' in roboflow_data:
                        nestle_count = roboflow_data['counts'].get('nestle', nestle_count)
                        comp_count = roboflow_data['counts'].get('competitor', comp_count)

                # Clean up the paths to only include filenames
                clean_paths = [path.split('/')[-1] for path in image_paths]

                return jsonify({
                    'id': event['id'],
                    'device_id': event['device_id'],
                    'timestamp': event['timestamp'],
                    'labeled_images': clean_paths,
                    ...
                    'image_path': clean_paths[0] if clean_paths else None,
                    'created_at': event['created_at']
                })

            except json.JSONDecodeError as e:
                logger.error(f"JSON parsing error for event {event_id}: {e}")
                return jsonify({'error': 'Invalid data format in event'}), 500
                ...
                with get_db_connection() as conn:
                    ...
                    cursor = conn.execute(
                        ...
                        ('web_upload', current_time, json.dumps(detection_data), 
                         labeled_filename, current_time)  # Store only filename
                    )
                    event_id = cursor.lastrowid
                    conn.commit()
